...
app.get("/hotelrooms/:guestamount", async (req, res) => {
  try {
    ...
    const suitableRooms = await HotelRoom.find({ capacity: { $gte: amount } });
...
app.get(
  "/hotelrooms/booking/date/:date/guestamount/:guestamount",
  async (req, res) => {
    try {
      const date = new Date(req.params.date);
      const guest = parseInt(req.params.guestamount);

      const rooms = await HotelRoom.find({ capacity: { $gte: guest } });
...
app.post("/hotelrooms/booking/check-availability", async (req, res) => {
  try {
    ...
    const rooms = await HotelRoom.find({ type: { $eq: roomType } });
    ...
    for (let room of rooms) {
      for (
        ...
        const roomStatusDoc = await RoomStatus.findOne({
          $and: [{ roomId: { $eq: room.id } }, { date: { $eq: date } }],
        });
    ...
    return res
      .status(404)
      .json({ error: "No available room for the given type and period" });
  } catch (error) {
  ...
  try {
    for (
      ...
      const roomStatus = await RoomStatus.findOne({
        $and: [{ roomId: { $eq: roomId } }, { date: { $eq: date } }],
      });
    ...
    res
      .status(500)
      .json({ message: "An error occurred while updating the room status" });
  }
});
...
app.post("/booking", async (req, res) => {
  try {
    const { userId, roomId, checkinDate, checkoutDate, guests, price } =
      req.body;
