...
export const RoomResults = ({ rooms, checkinDate }) => {
  ...
  if (!rooms || rooms.length === 0 || filteredRooms.length === 0) {
    return (
      <Container>
        <p>
          No rooms available for the selected date and guest amount. Please
          choose another date or room.
        </p>
      </Container>
    );
  }
    ...
    fetch(
      ...
      .then((data) => {
        navigate("/bookingconfirm", {
          state: {
            successMessage:
              "Room is available under this period. Please confirm your booking details and register your account.",
            roomType,
            checkinDate,
            checkoutDate,
